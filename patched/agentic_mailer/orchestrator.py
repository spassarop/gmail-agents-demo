...
class Orchestrator:
    ...
    def handle_chat(self, session: SessionState, user_message: str) -> ChatResponse:
        with traced(
            ...
            if tool_req is None:
                ...
                self._append_trace(trace, "heuristic_fallback", {"tool": getattr(tool_req, 'action', None) if tool_req else None})
            ...
            self._append_trace(
                ...
                "tool_request_validated",
                {
                    ...
                    "args": getattr(tool_req, "args", None).model_dump() if hasattr(getattr(tool_req, "args", None), "model_dump") else {},
                },
            )
    ...
    def _list_emails(self, session: SessionState, req: ListEmailsRequest, trace: List[TraceEvent]) -> ChatResponse:
        with traced(
            ...
            self._append_trace(trace, "gmail_list_messages", {"count": len(items), "query": query, "max_results": a.max_results})
    ...
    def _summarize_email(self, session: SessionState, req: SummarizeEmailRequest, trace: List[TraceEvent]) -> ChatResponse:
        with traced(
            ...
            self._append_trace(trace, "summary_agent_structured", summary.model_dump())
    ...
    def _draft_email(self, session: SessionState, user_message: str, req: DraftEmailRequest, trace: List[TraceEvent]) -> ChatResponse:
        with traced(
            ...
            if a.reply_to_email_number:
                ...
                self._append_trace(trace, "composition_agent_structured", draft.model_dump())
            ...
            self._append_trace(trace, "gmail_create_draft", {"draft_id": gmail_draft.id, "to": to_email, "subject": subject})
    ...
    def _prepare_trash_with_confirmation(
        ...
        with traced(
            "security.hitl.prepare_trash",
            attributes={
                "app.mode": self.mode,
                "tool.request.action": "TRASH_EMAIL",
    ...
    def _prepare_send_with_confirmation(self, session: SessionState, req: SendEmailRequest, gate_reason: str, trace: List[TraceEvent]) -> ChatResponse:
        with traced(
            "security.hitl.prepare_send",
            attributes={
                "app.mode": self.mode,
                "tool.request.action": "SEND_EMAIL",
                "security.require_confirmation": True,
            },
        ):
            a = req.args
            ...
            self._append_trace(trace, "gmail_create_draft_for_send", {"draft_id": draft.id, "to": a.to_email, "subject": a.subject})
    ...
    def _confirm(self, session: SessionState, action_id: Optional[str], trace: List[TraceEvent]) -> ChatResponse:
        with traced(
            "security.hitl.confirm",
            attributes={
                "app.mode": self.mode,
                "hitl.action_id": action_id or "",
            },
        ):
            if not action_id:
